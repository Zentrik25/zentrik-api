...
- Create: Required fields for creating a booking
- Update: Optional fields for updating a booking
- Read: Complete booking data returned to client
"""
...
class BookingBase(BaseModel):
    ...
    client_name: str = Field(..., min_length=1, max_length=255, description="Client's full name")
    client_phone: str = Field(..., min_length=1, max_length=50, description="Client's contact phone")
    client_email: Optional[EmailStr] = Field(None, description="Client's email (optional)")
    service_type: Optional[str] = Field(None, max_length=100, description="Type of service being booked")
    scheduled_at: datetime = Field(..., description="When the appointment is scheduled (ISO 8601 format)")
...
class BookingUpdate(BaseModel):
    ...
    """
    client_name: Optional[str] = Field(None, min_length=1, max_length=255, description="Updated client name")
    client_phone: Optional[str] = Field(None, min_length=1, max_length=50, description="Updated phone")
    client_email: Optional[EmailStr] = Field(None, description="Updated email")
    service_type: Optional[str] = Field(None, max_length=100, description="Updated service type")
